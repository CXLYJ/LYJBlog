...
public class BlogIndex {
	...
	public List<Blog> searchBlog(String q) throws Exception{
		...
		Fragmenter fragmenter =new SimpleSpanFragmenter(scorer); //得分高的片段设置进去
		...
		for(ScoreDoc score : hits.scoreDocs) {
			...
			if(content != null) {
				...
				if(StringUtil.isEmpty(hContent)) {
					if(content.length() > 200) { //如果没查到且content内容又大于100的话
						blog.setContent(content.substring(0, 200)); //截取100个字符
					} else {
